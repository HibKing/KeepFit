...
public class LoginService {
    public void selectLogin(String account,String password)
    {
        OkHttpClient okHttpClient = new OkHttpClient();
        RequestBody requestBody = new FormBody.Builder().add("uaccount",account).add("upassword",password)
                .build();
        Request request = new Request.Builder()
                .url("http://wangchaoyi.gz01.bdysite.com/keepfit/user_select.php")
                .post(requestBody)
                .build();
    ...
    private void parseAllShopping(Response response) throws IOException {
        ...
        try{
            ...
            for(int i=0;i<jsonArray.length();i++) {
                ...
                if (result.equals("1")) {
                    EventBus.getDefault().post(new MessageEvent("login_success","查询成功"));
                    return;
                }else {
                    EventBus.getDefault().post(new MessageEvent("login_false","暂无记录"));
                    return;
                }
            }
